...
##Diff Analysis
  ...
  comparative_statistics <- function(df) {
    ...
    log2FC <- log2(FC)
    Pval <- sigtest$p.value
    Method <- sigtest$method
    BH_Pval <- sigtest$bh_p.value
    Qval <- sigtest$q.value
    ShapE <- sigtest$'ShapE$p.value'
    ShapC <- sigtest$'ShapC$p.value'
    comp <- data.frame(FC, log2FC, Pval, Method, BH_Pval, Qval, ShapE, ShapC)
...
# t.test(trans_list$Set_1$HPRA042769, )
# ?t.test
# tset = trans_list$Set_1
# c = tset[tset$group==0,][['HPRA042769']]
# e = tset[tset$group==1,][['HPRA042769']]
# t.test(c,e)
# data[data$group==0,][[name]]
# trans_list_rsn$Set_1$HPRA042769
# any(is.na(trans_list$Set_1$HPRA042769))
# tb = lapply(dflist, transformer_boxcox)
# class(P02_Merged$Set_1)
# df = P02_Merged$Set_1
# tb1 = transformer_boxcox(P02_Merged$Set_1)    
